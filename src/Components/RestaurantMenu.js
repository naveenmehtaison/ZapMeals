...
const RestaurantMenu = () => {
  ...
  return (
    <div className=" overflow-hidden pt-0 p-4 mx-10 z-10  border-black ">
      <h1 className="text-center border-l-slate-950 font-extrabold">
        {redata.page_info.pageTitle} Restaurant menu
      </h1>
      <hr></hr>
      <div
        ...
        <div className="flex flex-col gap-2 overflow-y-scroll ">
          {redata.page_data?.order?.menuList?.menus.map((ele, item) => (
            ...
            >
              <h2
                style={{
                  color: menu?.menu?.id === ele?.menu?.id ? "red" : "black",
                  background:
                    menu?.menu?.id === ele?.menu?.id
                      ? "linear-gradient(90deg, rgb(255, 255, 255), rgb(255, 237, 239))"
                      : "white",
                }}
                className="font-semibold rounded-md p-2 w-full"
              >
                {ele.menu.name}
              </h2>
